<template>
  <div>
    <div v-if="presentationSessionInfo && !selectedCredential" id="content">
      ...
      <div
        class="_scrollable _container d-flex flex-column align-items-center justify-content-center">
        <div v-if="presentableCredentials.length > 0">
          <div
            class="form-check col-md-9 col-sm-12 mb-3"
            v-for="credential in presentableCredentials"
            :key="credential.id">
            <div class="row justify-content-center">
              <div class="col-2">
                <input
                  ...
                  :value="credential.id"
                  v-model="checkedCredentialIds" />
              </div>
              <div class="col-10">
                <a @click="selectedCredential = credential">{{
                  credential.title
                    ? credential.title
                    : $t(
                        'CREDENTIAL.TYPE.' +
        ...
        <div
          v-if="
            presentableCredentials.length === 0 && requiredSchemaIds.length > 0
          "
          id="content">
        ...
        <div
          ...
          v-if="
            presentableCredentials.length > 0 || requiredSchemaIds.length === 0
          ">
          <button href="#share" class="_share col-12 mb-2" @click="peSubmit()">
            Share
          </button>
        ...
        <div
          class="mt-4"
          v-if="
            presentationSessionInfo.availableIssuers != null &&
            presentationSessionInfo.availableIssuers.length > 0
          ">
          <div><b>Available issuers:</b></div>
          <select class="form-select" v-model="selectedIssuer">
            <option
              v-for="issuer in presentationSessionInfo.availableIssuers"
              :key="issuer.id"
              :value="issuer.id">
              {{ issuer.description }}
            </option>
          ...
          <div class="_button">
            <button
              href="#fetch"
              class="_share col-12 mb-2"
              @click="fetchFromIssuer()"
    ...
    <div v-if="selectedCredential">
      <div
        class="_top-bar position-sticky d-flex justify-content-center col-12 align-items-center px-3">
        <div class="col-12 d-flex align-items-center _animation-fade">
          <a
            href=""
            @click.prevent="selectedCredential = null"
            class="_back-button">
    ...
    <form
      ref="responseForm"
      method="post"
      :action="presentationSessionInfo.req.redirect_uri">
...
export default {
  ...
  async asyncData({ $axios, query, store }) {
    if (query.sessionId !== null) {
      const presentationSessionInfo = await $axios.$get(
        '/api/wallet/siopv2/continuePresentation',
        { params: { ...query, did: store.state.wallet.currentDid } },
      )
      let presentableCredentials = []
      if (presentationSessionInfo.presentableCredentials.length > 0) {
        const params = new URLSearchParams()
        presentationSessionInfo.presentableCredentials
          .map((c) => c.credentialId)
          .forEach((id) => params.append('id', id))
        const requestCfg = {
          params,
        }
        const presentableCredentialsList = await $axios.$get(
  ...
  methods: {
    async peSubmit() {
      const selectedPresentableCredentials =
        this.presentationSessionInfo.presentableCredentials.filter(
          (c) =>
            this.checkedCredentialIds.findIndex(
              (id) => id === c.credentialId,
            ) >= 0,
        )
      if (this.presentationSessionInfo.isPassiveIssuanceSession) {
        const issuanceSessionId = await this.$axios.$post(
          '/api/wallet/siopv2/fulfillPassiveIssuance',
          selectedPresentableCredentials,
          { params: { sessionId: this.presentationSessionInfo.id } },
        )
        this.$router.push(`/ReceiveCredential?sessionId=${issuanceSessionId}`)
      } else {
        const siopResp = await this.$axios.$post(
          '/api/wallet/siopv2/fulfillPresentation',
          selectedPresentableCredentials,
          { params: { sessionId: this.presentationSessionInfo.id } },
        )
