    ...
    <div class="d-flex align-items-center justify-content-center _wizard mt-3">
      ...
      <div :class="this.wizardIndex === 1 ? '_left-fade' : 'hide'">
        <div class="_item">
          ...
          <p>
            If no domain is entered, the did:web will be registered at walt.id.
          </p>
        </div>
        <div class="mt-3 d-flex _button-view justify-content-center">
          <div>
            <form
              action=""
              id="generate-did-submit"
              @submit.prevent="DIDgenerate">
              <input
                placeholder="Insert the domain (optional)"
                id="inserted-domain"
                name="insertedDomain"
                class="form-control _domain-form"
                style="width: 12em"
                :data="this.domain"
                v-model="domain" />
              <button
                type="submit"
                name="submit"
                class="_bounce btn text-white mt-3"
                style="width: 200px">
      ...
      <div :class="this.DIDgenerated === true ? '_fadin' : 'hide'">
        <div class="_item">
          ...
          <textarea
            ...
            cols="30"
            rows="5"
            class="mt-2 _did-content"
            :data="this.didContent"
            v-model="this.didContent"></textarea>
          <p
            v-if="coppied"
            class="text-secondary _bounce"
          ...
          <button
            type="button"
            @click="onCopy"
            class="_bounce btn _btn-copy text-white mt-2">
            <i class="bi bi-files me-2"></i>Copy DID
          </button>
