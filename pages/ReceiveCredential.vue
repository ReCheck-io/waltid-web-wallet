<template>
  <div>
    <div
      ...
      v-if="issuanceSessionInfo && !selectedCredential">
      ...
      <div
        ...
        <a
          v-for="credential in issuanceSessionInfo.credentials"
          :key="credential.id"
          class="_card d-flex _animation-fade"
          href="#selectCredential"
        ...
        <div class="_button">
          <button href="#share" class="_share col-12 mb-2" @click="accept()">
            {{ $t('RECEIVE_CREDENTIALS.ACCEPT') }}
          </button>
          <a href="#reject" class="_reject col-12">{{
            $t('RECEIVE_CREDENTIALS.REJECT')
          }}</a>
        </div>
      </div>
    </div>
...
export default {
  ...
  async asyncData({ $axios, query }) {
    if (query.sessionId !== null) {
      const issuanceSessionInfo = await $axios.$get(
        '/api/wallet/siopv2/issuanceSessionInfo',
        { params: query },
      )
  ...
  methods: {
    async accept() {
      ...
      if (
        this.issuanceSessionInfo.issuanceRequest.walletRedirectUri !== null &&
        this.issuanceSessionInfo.issuanceRequest.walletRedirectUri !== ''
      ) {
